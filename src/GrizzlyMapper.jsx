...
const DirectEditor = ({ mapping, onChange, onOpenSidebar }) => (
  <div className="space-y-2">
    ...
    <div className="flex items-center gap-2">
      <div className="relative flex-1">
        ...
        <input
          ...
          placeholder="Click to select source field..."
        />
      </div>
    </div>
    ...
    <select
      ...
      className="w-full text-xs border border-slate-200 rounded px-2 py-1.5 bg-white focus:ring-1 focus:ring-blue-300 outline-none"
    >
  ...
  <div className="space-y-3">
    ...
    <div className="grid grid-cols-3 gap-2">
      <div className="relative">
        ...
        <input
          value={mapping.condField || ""}
          onChange={(e) => onChange({ condField: e.target.value })}
          onClick={() => onOpenSidebar("source", "condField")}
          placeholder="field"
          className="w-full pl-7 text-xs font-mono border border-slate-200 rounded px-2 py-1.5 cursor-pointer hover:bg-blue-50"
        />
      </div>
      <select
        value={mapping.condOp || "=="}
        onChange={(e) => onChange({ condOp: e.target.value })}
      ...
      <input
        value={mapping.condValue || ""}
        onChange={(e) => onChange({ condValue: e.target.value })}
        placeholder="value"
        className="text-xs font-mono border border-slate-200 rounded px-2 py-1.5"
      />
    </div>
...
const TRANSFORMATION_PLUGINS = {
  direct: { id: "direct", label: "Direct", Editor: DirectEditor, generate: (m) => { const src = m.source || "source"; let code = `INPUT.${src}`; if (m.transform === "upper") code += ".upper()"; else if (m.transform === "lower") code += ".lower()"; else if (m.transform === "capitalize") code += ".capitalize()"; else if (m.transform) code = `${m.transform}(${code})`; return code; } },
  conditional: { id: "conditional", label: "If/Else", Editor: ConditionalEditor, generate: (m) => { const field = m.condField || "field"; const op = m.condOp || "=="; const val = m.condValue || "value"; const then = m.thenValue || "value1"; const els = m.elseValue || "value2"; return `"${then}" if INPUT.${field} ${op} "${val}" else "${els}"`; } }
};
...
const FieldBrowserSidebar = ({ fields, title, mode, onClose, onSelect }) => { const [search, setSearch] = useState(""); const filtered = fields.filter(f => f.path.toLowerCase().includes(search.toLowerCase())); return (<div className="w-80 bg-white border border-slate-200 rounded-xl shadow-lg overflow-hidden flex flex-col" style={{maxHeight: "600px"}}><div className="px-4 py-3 border-b border-slate-100 flex justify-between items-center bg-slate-50"><h3 className="font-bold text-sm text-slate-700">{title}</h3><button onClick={onClose} className="text-slate-400 hover:text-slate-600"><X size={16}/></button></div><div className="p-3 border-b border-slate-100"><div className="relative"><Search size={14} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-400"/><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search fields..." className="w-full pl-8 pr-2.5 py-1.5 text-xs border border-slate-200 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-300"/></div></div><div className="flex-1 overflow-y-auto p-2">{filtered.map(f => (<button key={f.path} onClick={() => onSelect(f.path)} className="w-full text-left px-3 py-2 text-xs font-mono hover:bg-blue-50 rounded-md text-slate-600 hover:text-blue-600 transition-colors flex items-center gap-2">{f.type === "object" ? <Type size={12} className="text-slate-400"/> : <Database size={12} className="text-blue-400"/>}<span className="flex-1">{f.path}</span><span className="text-[9px] text-slate-300">{f.type !== "object" && f.type}</span></button>))}</div></div>); };
...
export default function GrizzlyMapper() {
  ...
  const handleFile = (type, file) => { const reader = new FileReader(); reader.onload = (e) => { try { const content = e.target.result; if (type === "input") { const schema = JSON.parse(content); setInputSchema(schema); setInputFields(extractFields(schema)); } else if (type === "output") { const schema = JSON.parse(content); setOutputSchema(schema); setOutputFields(extractFields(schema)); } else if (type === "template") { parseExistingTemplate(content); } } catch (err) { alert("Invalid file format"); } }; reader.readAsText(file); };

  const parseExistingTemplate = (content) => { const lines = content.split('\n'); const parsedMappings = []; lines.forEach(line => { const match = line.match(/OUTPUT\["([^"]+)"\]\s*=\s*INPUT\.([^\s.]+)(?:\.(\w+)\(\))?/); if (match) { const [, target, source, transform] = match; parsedMappings.push({ id: uid(), target, source, transform: transform || null, transformation: "direct", isNew: false }); } }); if (parsedMappings.length > 0) { updateModuleMappings(activeModule, parsedMappings); } };
  ...
  const handleSidebarSelect = (path) => { if (sidebarState.rowIdx !== null) { if (sidebarState.field) { updateMapping(sidebarState.rowIdx, { [sidebarState.field]: path }); } else { const key = sidebarState.mode === "target" ? "target" : "source"; updateMapping(sidebarState.rowIdx, { [key]: path }); } } setSidebarState({ ...sidebarState, isOpen: false }); };
  ...
  const generateCode = () => { const lines = []; lines.push("#!/usr/bin/env python3"); lines.push('"""'); lines.push("Generated by Grizzly"); lines.push('"""'); lines.push(""); modules.forEach((module, idx) => { if (module.mappings.length === 0) return; if (module.name === "main") { lines.push("def transform(INPUT):"); lines.push('    """Main transformation function"""'); lines.push("    OUTPUT = {}"); lines.push("    "); } else { lines.push(`def map_${module.name.replace(/[^a-z0-9]/gi, "_").toLowerCase()}(INPUT, OUTPUT):`); lines.push(`    """${module.name} mappings"""`); lines.push("    "); } module.mappings.forEach(m => { const Plugin = TRANSFORMATION_PLUGINS[m.transformation]; if (!Plugin || !m.target) return; const targetPath = m.target.split("."); const code = Plugin.generate(m); let assignment = ""; if (targetPath.length === 1) { assignment = `    OUTPUT["${targetPath[0]}"] = ${code}`; } else { const parts = targetPath.map(p => `["${p}"]`).join(""); assignment = `    OUTPUT${parts} = ${code}`; } lines.push(assignment); }); if (module.name === "main") { lines.push("    "); lines.push("    return OUTPUT"); } lines.push(""); }); return lines.join("\n"); };
